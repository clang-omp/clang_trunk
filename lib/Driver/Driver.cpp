...
void Driver::BuildActions(const ToolChain &TC, DerivedArgList &Args,
    ...
    for (SmallVectorImpl<phases::ID>::iterator
      ...
      for (unsigned tgt=TotalTargets; tgt>0; --tgt){
        ...
        if ( PreviousTotalTargets < TotalTargets && tgt>1 )
          CurrentInput.reset(ActionsForTarget[0].get());
        else
          CurrentInput.reset(ActionsForTarget[tgt-1].release());
  ...
  for (unsigned tgt = OpenMPTargets.size(); tgt > 0; --tgt){
    ...
    TgtLinkAction.reset(
        new BindTargetAction(std::move(TgtLinkAction), OpenMPTargets[tgt-1]));
