...
void CodeGenFunction::EmitOMPDirectiveWithParallelNoMicrotask(
  ...
  if (isTargetMode)
    CGM.getOpenMPRuntime().StartParallelRegionInTarget(*this, DKind, SKinds, S);
  ...
  if (!Fn){
    ...
    // This will complete the initialization done before
    {
      ...
      if (isTargetMode)
        ...
        CGM.getOpenMPRuntime().GenerateFinishLabelSetting(CS->getLocStart(),
                                                          CGF, false);
